    ...
    for feed in feeds:
        ...
        for report in importer.import_new(start_filter=datetime(2012,1,1)):
            ...
            for notice in report.notices:
                if isinstance(notice,EventImportReport.RecordExists):
                    exists_count += 1
                else:  
                    importlog.info(u'UID %s: Report status notice: %s' % (record.uid,notice))

        importlog.info('%d new Events imported from feed "%s" (%d existing skipped)' % (import_count,unicode(feed),exists_count))
