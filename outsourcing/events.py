...
from onlyinpgh.outsourcing.apitools.facebook import BatchCommand, FacebookAPIError
from onlyinpgh.outsourcing.apitools.facebook import oip_client as fb_client

from onlyinpgh.events import categorize

from onlyinpgh import outsourcing
...
from onlyinpgh.utils import time
...
class EventProcessingRecord:
    '''Struct to keep in-progress info when adding events'''
...
class FBEventPuller:
    class PageRecord:
        '''Struct to keep in-progress info when adding pages'''
        def __init__(self,page_id):
            self.fb_id = page_id
            self.linked_place = None
            self.linked_org = None
            self.resolved_place_cache = []
            self.errors = []
            self.event_records = []        

    def __init__(self):
        self.page_records = {}      # indexed by fb id
        self.event_records = {}     # indexed by fb id

        def add_page_ids(self,page_ids):
            '''Adds a batch of page ids to process (won't overwrite existing)'''
            for pid in page_ids:
                if pid not in self.page_records:
                    self.page_records[pid] = PageRecord(pid)

        def process_all(self):
            '''
            Cycle through all stored pages and create events for all of them.
            '''
            # fill in all the non-existant page_info entries
            # grab all events for each page using code from gather_event_info
            self.event_records = gather_event_info( [ p.fb_id for p in self.page_records ] )
            for r in self.event_records:
                event_info = r.fb_info
                event_id = r.fb_id
                pic = f.picture

@transaction.commit_on_success
def store_fbevent(event_info,event_image=None,related_page_infos=None,
                    create_place=True,create_owner=True):
    '''
    ...
    related_page_infos is a list of page_info dicts that will be checked 
    before querying the live service when more detailed information is 
    needed to create additional related model instances (i.e. Place or 
    Organizations).

    If create_owner or create_place is True, the respective model instance 
    ...
    if referer_fbid:
        ...
        try:
            fbevent = (e for e in referer_events if e['id'] == fbid).next()
    ...
    if fbevent:
        ...
        try:
            ...
            fbevent = fb_client.graph_api_objects_request(fbid)
    ...
    if 'name' not in fbevent:
        dbglog.error('no name for event with fbid %s' % fbid)
        return None
    ...
    try:
        ...
        dblog.error('bad start time (%s) for event fbid %s' % (fbevent.get('start_time'),fbid))
        return
    try:
        ...
        dblog.error('bad end time (%s) for event fbid %s' % (fbevent.get('end_time'),fbid))
        return

    # wrap this in something else
    import urllib
    try:
        event.image_url = urllib.urlopen('http://graph.facebook.com/%s/picture?type=normal'%fbid).url
    except IOError as e:
        ...
        if place:
            ...
            try:
                place,created = Place.objects.get_or_create(
                                        ...
                                        location=location)
                if created:
                    dbglog.info('created new place for fbid %s: "%s"' % (fbid,debug_place_print(place)))
                else:
                    dbglog.info('retreived existing place for fbid %s: "%s"' % (fbid,debug_place_print(place)))
                save_to_event_cache(venue,place_name,place)
            except Warning as w:
                dbglog.warning('while saving place for fbid %s: %s' % (fbid,str(w)))
    ...
    try:
        ...
        estr = '<UNICODE ERROR>'
    dbglog.info('created new event for fbid %s: "%s"' % (fbid,estr))
    ...
    if fbowner.get('id'):
        ...
        dbglog.notice('no owner listed for fbid %s' % fbid)
...
# TODO: make this return a list of event_records
